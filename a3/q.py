...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 12 18:48:59 2019
...
def q(grid_world, q_s_a, epsilon=0.1, num_episodes=10000, sample_rate=10):
    ep_length_log = []
    time_log = []
    ...
    for ep in range(num_episodes):
        ...
        while not grid_world.terminated:
            ...
            q_s_a[state, action] = q_s_a[state, action] + args.alpha*(reward + args.discount_factor*max(q_s_a[succ_state]) - q_s_a[state, action])
...
def main(arguments):
    ...
    for i in range(num_runs):
        ...
        if not args.use_ep_func:
             _,ep_length_log, time_log, avg_ep_length_log, avg_time_log = q(grid_world, q_s_a, args.epsilon, num_episodes=num_episodes)
        else:
             _,ep_length_log, time_log, avg_ep_length_log, avg_time_log = q(grid_world, q_s_a, epsilon_func, num_episodes=num_episodes)
