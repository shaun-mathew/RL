...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 12 20:12:19 2019
...
def expected_sarsa(grid_world, q_s_a, epsilon=0.1, num_episodes=10000, sample_rate=10):
    ep_length_log = []
    time_log = []
    ...
    for ep in range(num_episodes):
        ...
        while not grid_world.terminated:
            action, eps = get_action(q_s_a,state,epsilon=epsilon,t=ep)

            succ_state, reward = grid_world.move(action)
            succ_state = grid_world.state_to_num[succ_state]

            max_action = get_max_action(q_s_a, succ_state)

            v_succ = (1 - eps)*q_s_a[succ_state,max_action] + (eps/4)*(sum(q_s_a[succ_state, a] for a in range(4)))

            q_s_a[state, action] = q_s_a[state, action] + args.alpha*(reward + args.discount_factor*v_succ - q_s_a[state, action])
...
def main(arguments):
    ...
    for i in range(num_runs):
        ...
        if not args.use_ep_func:
             _,ep_length_log, time_log, avg_ep_length_log, avg_time_log = expected_sarsa(grid_world, q_s_a, args.epsilon, num_episodes=num_episodes)
        else:
             _,ep_length_log, time_log, avg_ep_length_log, avg_time_log = expected_sarsa(grid_world, q_s_a, epsilon_func, num_episodes=num_episodes)
